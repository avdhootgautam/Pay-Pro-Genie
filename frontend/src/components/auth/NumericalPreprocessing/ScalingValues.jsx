// Here i will create the component for Scaling the values
import { Box , Typography,Button} from "@mui/material"
const ScalingValues=({numericalPreproStep,setStrategy,strategy,columnPrepro,setColumnPrePro,finalOperation,setApplyAndSaveButton})=>{
    ...
    return (
        <Box  sx={{color:"whitesmoke",paddingTop:"2px",paddingLeft:"10px",display:"flex",flexDirection:"column"}}>
            <Box sx={{display:"flex",flexDirection:"column"}}>
                <Box sx={{height:"40px"}}><h3>Scale features and remove outliers</h3></Box>
                <Box sx={{paddingBottom:"5px"}}><Typography sx={{fontSize:"14px",fontWeight:300,color:'gray'}}>Standard, Min-Max, Robust, or Max-Abs scaling.</Typography></Box>
            </Box>
            <Box sx={{display:"flex",flexDirection:"row",paddingTop:"5px"}}>
                <Box sx={{display:"flex",flexDirection:"column"}}>
                    <Typography sx={{fontSize:"14px",fontWeight:300,color:'gray'}}>SCALER</Typography>
                    <Box sx={{paddingTop:"5px"}}>
                        <Button sx={strategyBtnStyle(strategy==="Standard")} onClick={()=>{handleStrategyButton("Standard")}}>Standard</Button>
                        <Button sx={strategyBtnStyle(strategy==="Min-Max")} onClick={()=>{handleStrategyButton("Min-Max")}}>Min-Max</Button>
                        <Button sx={strategyBtnStyle(strategy==="Robust")} onClick={()=>{handleStrategyButton("Robust")}}>Robust</Button>
                        <Button sx={strategyBtnStyle(strategy==="Max Absolute")} onClick={()=>{handleStrategyButton("Max Absolute")}}>Max Absolute</Button>
                    </Box>
                </Box>
